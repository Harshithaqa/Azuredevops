...
class HtmlReporter extends reporter_1.default {
    ...
    onSuiteStart(suite) {
        ...
        this.options.LOG.info(typescript_string_operations_1.String.Format("onSuiteStart: {0}", suite.cid));
        this.options.LOG.debug(JSON.stringify(thisSuite));
    }
    onTestStart(theTest) {
        this.options.LOG.info(typescript_string_operations_1.String.Format("onTestStart: {0}", theTest.cid));
    ...
    onTestPass(test) {
        this.options.LOG.info(typescript_string_operations_1.String.Format("onTestPass: {0}", test.cid));
        this.options.LOG.debug(JSON.stringify(test));
        this.metrics.passed++;
    }
    onTestSkip(test) {
        this.options.LOG.info(typescript_string_operations_1.String.Format("onTestSkip: {0}", test.cid));
        this.options.LOG.debug(JSON.stringify(test));
        this.metrics.skipped++;
    }
    onTestFail(theTest) {
        this.options.LOG.info(typescript_string_operations_1.String.Format("onTestFail: {0}", theTest.cid));
        this.options.LOG.debug(JSON.stringify(theTest));
    ...
    onTestEnd(theTest) {
        this.options.LOG.info(typescript_string_operations_1.String.Format("onTestEnd: {0}", theTest.cid));
        this.options.LOG.debug(JSON.stringify(theTest));
        let test = this.getTest(theTest.uid);
        if (test) {
            this.moveErrorsToEvents(test);
    ...
    onSuiteEnd(suite) {
        this.options.LOG.info(typescript_string_operations_1.String.Format("onSuiteEnd: {0}", suite.cid));
    ...
    onAfterCommand(command) {
        if (this.options.useOnAfterCommandForScreenshot) {
            if (this.isScreenshotCommand(command) && command.result.value) {
                ...
                this.options.LOG.info(typescript_string_operations_1.String.Format("onAfterCommand: {0} taking screenshot {1}", this._currentTestUid, filepath));
